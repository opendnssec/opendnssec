#!/bin/sh
#
# $Id: build_and_test.sh 4425 2011-02-08 16:50:21Z matthijs $
...
SRCDIR=${WORKSPACE}/src/trunk/OpenDNSSEC
OBJDIR=${WORKSPACE}/obj

LIBSOFTHSM=/home/jenkins/workspace/A_SoftHSM_test_and_install_Needed_by_other_projects/label/$NODE_NAME/lib/libsofthsm.so

SOFTHSM_TEMPLATE=${OBJDIR}/test/conf/softhsm.conf
SOFTHSM_CONF=${SANDBOX}/etc/opendnssec/softhsm.conf
export SOFTHSM_CONF

export PATH=/usr/local/bin:/home/jenkins/workspace/A_SoftHSM_test_and_install_Needed_by_other_projects/label/$NODE_NAME/bin:/usr/ccs/bin:/usr/gnu/bin:$PATH
...
build_opendnssec()
{
	...
	(cd ${OBJDIR}; ${SRCDIR}/configure \
		--prefix=${SANDBOX} ${CONFIGURE_ARGS} --enable-auditor CFLAGS=-std=c99)
	rc=$?
	if [ $rc != 0 ]; then
		exit $rc
	fi
...
setup_softhsm()
{
	...
	LD_LIBRARY_PATH=/usr/local/lib softhsm --init-token \
		--slot $SLOT \
		--pin $PIN \
		--so-pin $SO_PIN \
		--label $LABEL
...
ods_start()
{
        ...
        sleep 10
        ${SANDBOX}/sbin/ods-signer verbosity 4
