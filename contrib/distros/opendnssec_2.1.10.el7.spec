...
if [ -z "$(%{_bindir}/sqlite3 %{_localstatedir}/opendnssec/kasp.db 'select * from databaseVersion;')" ]; then
   # Migrate version 1.4 db to version 2.1 db
   if [ -d %{_localstatedir}/opendnssec/tmp ]; then
      rmdir %{_localstatedir}/opendnssec/signer
   ...
   if [ -e %{_localstatedir}/opendnssec/rpm-migration-in-progress ]; then
      ...
      if [ -n "$ERR" ]; then
         echo "%{_localstatedir}/opendnssec/kasp.db migration failed, human intervention is needed"
      fi
   fi
fi
