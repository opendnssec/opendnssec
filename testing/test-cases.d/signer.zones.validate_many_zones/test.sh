...
echo -n "LINE: ${LINENO} " && ods_setup_zone test/all.rr.org &&
echo -n "LINE: ${LINENO} " && num_signedzones=`syslog_grep_count2 'ods-signerd: .*\[STATS\] all.rr.org'` &&
echo -n "LINE: ${LINENO} " && log_this_timeout ods-update-zone 20 ods-signer sign all.rr.org &&
echo -n "LINE: ${LINENO} " && syslog_waitfor_count 60 `expr $num_signedzones + 1` 'ods-signerd: .*\[STATS\] all.rr.org' &&
echo -n "LINE: ${LINENO} " && test -f "$INSTALL_ROOT/var/opendnssec/signed/all.rr.org" &&
...
echo -n "LINE: ${LINENO} " && sleep 5 &&

#OPENDNSSEC-247 - Update the SOA minimum in the policy and make sure the NSEC TTL changes.
echo -n "LINE: ${LINENO} " && $GREP -q -- "<Minimum>PT5M</Minimum>" "$INSTALL_ROOT/var/opendnssec/signconf/all.rr.org" &&
echo -n "LINE: ${LINENO} " && $GREP -q -- "300.*IN.*NSEC3" "$INSTALL_ROOT/var/opendnssec/signed/all.rr.org" &&
echo -n "LINE: ${LINENO} " && cp kasp.xml kasp.xml_orig &&
echo -n "LINE: ${LINENO} " && cp test/kasp.xml kasp.xml &&
echo -n "LINE: ${LINENO} " && log_this ods-update-policy ods_setup_conf kasp.xml &&
echo -n "LINE: ${LINENO} " && log_this_timeout ods-update-policy 10 ods-enforcer policy import &&
echo -n "LINE: ${LINENO} " && #syslog_waitfor 300 'ods-enforcerd: .*Called signer engine:.*ods-signer update all.rr.org' &&
echo -n "LINE: ${LINENO} " && syslog_waitfor 300 'ods-enforcerd: .*\[enforcer\] updateZone: processing all.rr.org with policyName default' &&
echo -n "LINE: ${LINENO} " && syslog_waitfor 300 'ods-signerd: .*zone all.rr.org scheduled for immediate .*' &&
echo -n "LINE: ${LINENO} " && sleep 5 &&
echo -n "LINE: ${LINENO} " && $GREP -q -- "<Minimum>PT10M</Minimum>" "$INSTALL_ROOT/var/opendnssec/signconf/all.rr.org" &&
echo -n "LINE: ${LINENO} " && syslog_waitfor_count 300 `expr $num_signedzones + 2` 'ods-signerd: .*\[STATS\] all.rr.org' &&
echo -n "LINE: ${LINENO} " && test -f "$INSTALL_ROOT/var/opendnssec/signed/all.rr.org" &&
echo -n "LINE: ${LINENO} " && $GREP -q -- "600.*IN.*NSEC3" "$INSTALL_ROOT/var/opendnssec/signed/all.rr.org" &&
