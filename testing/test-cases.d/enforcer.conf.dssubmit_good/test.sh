...
KSK_CKA_ID=`log_grep -o ods-enforcer-cka_id stdout "ods[[:space:]]*KSK[[:space:]]*publish" | awk '{print $9}'` &&
...
syslog_grep "\[enforce_task\] please submit DS with keytag .* for zone ods" &&
