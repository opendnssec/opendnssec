...
echo -n "LINE: ${LINENO} " && count=`grep -Pc "RRSIG\tMX" "$INSTALL_ROOT/var/opendnssec/signed/ods"` &&
...
echo -n "LINE: ${LINENO} " && count=`grep -Pc "DNSKEY\t256" "$INSTALL_ROOT/var/opendnssec/signed/ods"` &&
...
echo -n "LINE: ${LINENO} " && grep -P "RRSIG\tMX" "$INSTALL_ROOT/var/opendnssec/signer/ods.backup2" | grep -v $ZSK2 | grep $ZSK1 &&
...
echo -n "LINE: ${LINENO} " && grep -P "RRSIG\tMX" "$INSTALL_ROOT/var/opendnssec/signer/ods.backup2"| grep -v $ZSK1 | grep $ZSK2 &&
echo -n "LINE: ${LINENO} " && ldns-verify-zone "$INSTALL_ROOT/var/opendnssec/signed/ods" &&
