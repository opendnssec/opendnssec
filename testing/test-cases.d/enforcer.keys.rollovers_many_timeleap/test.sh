...
KSK_CKA_ID1=`log_grep -o ods-enforcer-output stdout "ods1[[:space:]].*KSK[[:space:]].*ready[[:space:]]*waiting for ds-seen" | awk '{print $9}'` &&
...
KSK_CKA_ID2=`log_grep -o ods-enforcer-output_manual stdout "ods1[[:space:]].*KSK[[:space:]].*ready[[:space:]]*waiting for ds-seen" | awk '{print $9}'` &&

#### TIME 12: Expect KSK to be ready 
...
#### TIME 13: Expect KSK to be active 
...
log_this ods-enforcer-output_manual ods-enforcer key ds-gone --zone ods1 --cka_id $KSK_CKA_ID1 && sleep 1 &&
...
#### TIME 13.5: ???
log_this ods-enforcer-output_manual echo "--------------- TIME LEAP 13.5 -----------------" &&
#log_this ods-enforcer-output_manual 'ods-enforcer time leap' && 
sleep 1 &&
log_this ods-enforcer-output_manual echo "--------------------------------------------" &&
