...
static const char *policy_list_cmd_str = "policy_list_cmd";
                                ...
                                ssize_t n)
{
    ...
    ods_log_debug("[%s] %s command", policy_list_cmd_str, scmd);
    if (cmd[ncmd] == '\0') {
        cmd = "";
    } else if (cmd[ncmd] != ' ') {
        return 0;
    ...
    if (strncmp(cmd, "--task", 7) == 0) {
        /* schedule task */
        task = policy_list_task(engine->config);
        if (!task) {
            ods_log_crit("[%s] failed to create %s task",
                         policy_list_cmd_str,scmd);
        } else {
