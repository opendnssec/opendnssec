...
AC_DEFINE_UNQUOTED(ODS_SE_SIGNERTHREADS, [1],                                [Default number of signer threads for the OpenDNSSEC signer engine])
...
AC_DEFINE_UNQUOTED(ODS_EN_ENGINE,        ["$OPENDNSSEC_ENFORCER_ENGINE -vvv"],        [Path to the OpenDNSSEC enforcer engine binary])
